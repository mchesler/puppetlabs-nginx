...
#   [*force_ssl*]        - If 'true', non SSL requests will be forced to SSL
...
define nginx::resource::vhost(
  ...
  ## Add header stubs to force all traffic to SSL
  if ($force_ssl == 'true') {
    file { "${nginx::config::nx_temp_dir}/nginx.d/${name}-002":
      ensure => $ensure_real,
      content => template('nginx/vhost/vhost_header_force_ssl.erb'),
      notify => Class['nginx::service'],
    }
  }
